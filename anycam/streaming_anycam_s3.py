...
from open3d_pipeline import process_fusion_batch
...
def process_batch(model, frames, batch_index, video_name, s3_client, target_bucket, start_idx=0, pipeline=None):
    ...
    rgbs, depths, confidence, trajectory, proj_matrix = process_frames_with_anycam(model, frames)

    print(type(rgbs), type(depths), type(trajectory), type(proj_matrix))
    ply_bytes = process_fusion_batch(rgbs, depths, trajectory, proj_matrix)
