...
# Simple AnyCam Batched Video Comparison Script
...
show_help() {
    echo "Simple AnyCam Batched Video Comparison Tool"
    echo "==========================================="
    echo ""
...
echo "Simple AnyCam Batched Video Comparison"
echo "====================================="
...
# Create a simple Python script to handle the batch combination
...
def load_depth_from_batch(batch_dir, frame_index_in_batch):
    # Look for depth files
    depth_file = os.path.join(batch_dir, "depths.npy")
    if os.path.exists(depth_file):
        depths = np.load(depth_file)
        if len(depths.shape) == 4:  # (frames, height, width, channels)
            if frame_index_in_batch < depths.shape[0]:
                return depths[frame_index_in_batch, :, :, 0]
        elif len(depths.shape) == 3:  # (frames, height, width)
            if frame_index_in_batch < depths.shape[0]:
                return depths[frame_index_in_batch]
    return None

def apply_colormap(depth, colormap='viridis'):
    if depth is None:
        return None
...
def main():
    ...
    for i, batch_dir in enumerate(batch_dirs):
        ...
        for frame_in_batch in range(args.batch_size):
            global_frame = current_frame + frame_in_batch
...
if [ -f "$OUTPUT_VIDEO" ]; then
    echo ""
    echo "✅ Batched comparison video created successfully!"
    echo "📹 Output: $OUTPUT_VIDEO"
    echo ""
    echo "The video shows:"
    echo "  • Left side: Original frames"
    echo "  • Right side: Depth maps from batches (colormap: $COLORMAP)"
else
