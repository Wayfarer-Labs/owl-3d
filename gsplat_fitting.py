...
class Config:
    ...
    # Save rendered images during training/evaluation
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        data_directory = cfg.frames_dir if cfg.frames_dir is not None else cfg.data_dir
        self.parser = Parser(
        ...
        if cfg.max_frames is not None:
            ...
            original_len = len(self.trainset)
            self.trainset.parser.image_paths = self.trainset.parser.image_paths[:cfg.max_frames]
            self.trainset.parser.camtoworlds = self.trainset.parser.camtoworlds[:cfg.max_frames]
            if hasattr(self.trainset.parser, 'image_names'):
                self.trainset.parser.image_names = self.trainset.parser.image_names[:cfg.max_frames]
            print(f"Limited training frames from {original_len} to {len(self.trainset.parser.image_paths)}")
    ...
    def eval(self, step: int):
        ...
        for i, data in enumerate(valloader):
            ...
            # write images (conditional on save_imgs flag)
            if cfg.save_imgs:
